...
### Get file sample list (remove suffix)
  #ls ${DIR}/bams/*.sam | 's/\.[^.]*$//' > ${DIR}/samples.txt # starting after hisat
#ls ${DIR}/bams/*.bam | sed 's/\.[^.]*$//' > ${DIR}/samples.txt # starting with indexed/sorted bams (e.g., if already ran stringtie)
...
do
  ...
  echo "   genes..."
  date
  $SUBREAD/featureCounts -p -s 1 -T $THREADS -a $GTF -o genes/${ID}.counts $BAM 
    #-A $CHRALIAS # don't need?
  
  ### featureCounts: Exons
  echo "      exons..."
  date
  $SUBREAD/featureCounts -p -s 1 -T $THREADS -O -f -a $GTF -o exons/${ID}.counts $BAM
    #-A $CHRALIAS # don't need?
  ...
  $PKGS/bed_to_juncs_tophat.py < $OUTJXN > $OUTCOUNT ### Andrew uses: bed_to_junc_withCount (can't find it -- not in his github jaffelab R pkg; not in regtools...; might be synonymous with bed_to_juncs_tophat, from: https://github.com/infphilo/tophat/blob/master/src/bed_to_juncs)
