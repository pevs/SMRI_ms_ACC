...
### modified: 3 March 2018 *for PEC/PFC, which needs ALOT more threads*
...
#regionMat100_HPC <- regionMatrix(filteredCov_HPC, L = 100, runFilter = FALSE, verbose = TRUE)
...
#pheno <- read.delim("featureCounts_phenoData.txt", sep = "\t")
#files_PFC <- paste0("PFC/", as.character(pheno$FileName), ".bw")
#names(files_PFC) <- as.character(pheno$FileName)
...
### Filter coverage to reduce computational time of `regionMatrix`
# Using mean coverage of 5 reads (mean across all samples), for a region to be included as an expressed region
 # should check order same, so pheno$totalMapped in correct order!  
#filteredCov_PFC <- lapply(fullCov_PFC, filterData, 
#			returnMean = TRUE, filter = "mean", cutoff = 5, 
#			#totalMapped = pheno$totalMapped, targetSize = 8e+07 #80M libsize
#			verbose = TRUE)
#saveRDS(filteredCov_PFC, file = "filteredCov_PFC.rds")
filteredCov_PFC <- readRDS("filteredCov_PFC.rds")

### Calculate expressed region counts  
# L = read length (here, 100bp)
regionMat100_PFC <- regionMatrix(filteredCov_PFC, L = 100, runFilter = FALSE, returnBP = FALSE, verbose = TRUE)
